...
import random  # Import random module to set random seed
from loguru import logger

class BaseBoTorchOptimizer:
    
    def __init__(self, n_iterations, public_odd, real_probabilities, event, games_ids, df_probs_dict, seed=None):
...
class BoTorchOptimizer(BaseBoTorchOptimizer):
    ...
    def __init__(self, n_iterations, public_odd, real_probabilities, event, games_ids, df_probs_dict, seed=None, patience=10):
        super().__init__(n_iterations, public_odd, real_probabilities, event, games_ids, df_probs_dict, seed)
    ...
    def run_optimization(self):
        ...
        train_X = draw_sobol_samples(
            ...
            n=1,
            q=5,
            seed=self.seed,  # Use seed parameter
        ...
        for iteration in range(self.n_iterations):
            ...
            acq_func = LogExpectedImprovement(model=model, best_f=train_Y_standardized.max(), maximize=True)
            ...
            candidate, _ = optimize_acqf(
                ...
                options={'seed': self.seed}  # Use seed parameter
            )
            ...
            if self.no_improvement_counter >= self.patience:
                logger.info(f"No improvement in the last {self.patience} iterations. Stopping early at iteration {iteration}.")
...
class BoTorchOptimizerVariableStakeLambda(BaseBoTorchOptimizer):
    
    def __init__(self, n_iterations, public_odd, real_probabilities, event, games_ids, df_probs_dict, lambda_param, seed=None, patience=10):
        super().__init__(n_iterations, public_odd, real_probabilities, event, games_ids, df_probs_dict, seed)
    ...
    def compute_objective_via_analytical(
        ...
        my_second_moment = self.second_moment(allocation=x,
                                              ...
                                              df_probs_dict=df_probs_dict)
        #my_sigma = np.sqrt(self.variance(my_second_moment, my_expectation))
        my_variance = self.variance(my_second_moment, my_expectation)
        output = (1 - gamma + gamma * my_expectation) - self.lambda_param * gamma * my_variance
...
class BoTorchOptimizerLongTerm(BaseBoTorchOptimizer):
    """ This optimizer uses simulation for the objective function. """
    ...
    def compute_objective_via_simulation(
        ...
        x = x[:, 1:]
        x = F.softmax(x, dim=-1)
        # Set seeds for reproducibility in simulation
        if self.seed is not None:
            np.random.seed(self.seed)
            random.seed(self.seed)
    ...
    def run_optimization(self,
        ...
        for iteration in range(self.n_iterations):
            ...
            new_y = self.objective_function_simulation(candidate, df_prob, df_bet, num_simulations)
