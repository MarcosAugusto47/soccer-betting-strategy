...
for min_games in "${min_games_values[@]}"; do
  for bets_per_game in "${bets_per_game_values[@]}"; do
    for weight in "${weight_values[@]}"; do
      ...
      python main_callback_parallel_botorch.py --date_start 2022-01-01 --date_end 2023-01-01 --min_games "$min_games" --bets_per_game "$bets_per_game" --weight "$weight" --aggregator Datetime --n_iterations 50 --n_jobs 10 --save_experiment
    done
  done
done
