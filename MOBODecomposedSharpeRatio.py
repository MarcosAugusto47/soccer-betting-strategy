...
class MOBO:
                ...
                for j in range(n):
                    if i != j:
                        ...
                        if games_ids[i] == games_ids[j]:
                            ...
                            event_intersection_matrix_prob = (
                                event_intersection_matrix
                                * df_probs_dict[games_ids[i]].to_numpy()
                            )
                        ...
                        # P({A} intersection with {B}) = P({A}) * P({B})
                            ...
                            event_i_prob = (
                                event_i * df_probs_dict[games_ids[i]].to_numpy()
                            )
                            # Get probabilities matrix of event j of associated game, different from game above
                            event_j_prob = (
                                event_j * df_probs_dict[games_ids[j]].to_numpy()
                            )
    ...
    def compute_objective_via_analytical(
        ...
        my_expectation = self.expectation(
            allocation=x, public_odd=public_odd, real_probabilities=real_probabilities
        )
        
        output = my_expectation
    ...
    def compute_variance(
        ...
        my_second_moment = self.second_moment(
            allocation=x,
    ...
    def generate_initial_data(self, n=1):
        ...
        Y_init = torch.stack(
            [self.objective_1(X_init), self.objective_2(X_init)], dim=-1
        )
    ...
    def optimize_acqf_and_get_observation(self, acq_func):
        ...
        candidates, _ = optimize_acqf(
            ...
            bounds=torch.tensor([[0.0] * self.n, [10.0] * self.n]),
            q=1,
            num_restarts=10,
        ...
        new_obj = torch.stack(
            [self.objective_1(new_x), self.objective_2(new_x)], dim=-1
        )
    ...
    def run_optimization(self):
        ...
        for i in range(self.n_iterations):  # Number of iterations
            ...
            partitioning = NondominatedPartitioning(
                ref_point=torch.tensor([100.0, -1.]), Y=Y
            )
            acq_func = ExpectedHypervolumeImprovement(
                model=model, partitioning=partitioning, ref_point=[100.0, -1.]
            )
